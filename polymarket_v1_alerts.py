...
def coerce_outcomes(value: object) -> list[str]:
    """Gamma returns `outcomes` as a JSON-encoded string like '["Yes","No"]' in many cases.
    We normalize it to a list[str]. If parsing fails, return an empty list.
    """
    if value is None:
        return []
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            # Fallback: try to split naive CSV-ish strings
...
def fetch_gamma_markets(per_page: int, pages: int, *, active_only: bool = True) -> Tuple[List[MarketMeta], int]:
    ...
    for m in all_markets:
        ...
        # NOTE: do NOT filter on Gamma 'ready' (unreliable); keep market
        
        # Hard filter: markets that are not funded may not be tradable
        # NOTE: do NOT filter on Gamma 'funded' (unreliable); keep market
...
class MarketWatcher:
    ...
    def _handle_price_change(self, p: dict) -> None:
        ...
        for pc in p.get("price_changes", []) or []:
            ...
            # Optional guard: ignore huge % moves from near-zero prices (e.g., 0.002 → 0.004)
            # This makes Swing alerts only fire when the move starts above 1¢.
            if start.odds < 0.01:
                continue
    ...
    while True:
        ...
        if args.telegram:
            startup_msg = (
                ...
                f"📈 Swing threshold: {args.move_pct:.1f}% over {args.window_min}min\n"
                f"🧯 Min trade price for swing: ${args.min_trade_price:.2f}"
            )
